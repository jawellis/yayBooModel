...
// mp3 sounds
// const yaySound = new Audio('path/to/yay-sound.mp3');
// const booSound = new Audio('path/to/boo-sound.mp3');
...
//     let str = ""
    // in this code i only check thumb. check most important point and log if in right position. ex: if thumb[] < middlefinger[] && thumb[] < indexfinger[] = "well done"
...
function logData(predictions) {
    ...
    const indexFingerTip = annotations.indexFinger[3];
    const middleFingerTip = annotations.middleFinger[3];

    const messageElement = document.getElementById("message");

    if (thumbTip[1] < indexFingerTip[1] && thumbTip[1] < middleFingerTip[1]) {
        messageElement.innerText = "Great work"; // thumbs-up
        // yaySound.play(); // Play the "yay" sound
    } else if (thumbTip[1] > indexFingerTip[1] && thumbTip[1] > middleFingerTip[1]) {
        messageElement.innerText = "Lame!"; // thumbs-down
        // booSound.play(); // Play the "boo" sound
    } else {
        messageElement.innerText = ""; // reset message when no thumbs-up or thumbs-down is detected
    }
}
